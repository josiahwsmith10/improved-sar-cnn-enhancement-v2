classdef CrossEntropyJWS < nnet.internal.cnn.layer.RegressionLayer
    ...
    methods
        function this = CrossEntropyJWS(name)
        ...
        function loss = forwardLoss( this, Y, T )
            ...
            T(T==0) = 1e-40;
            crossentropy = -Y .* log10(abs(T));
            loss = sum( crossentropy (:) ) / numElements;
...
%             save T T
            ...
            T(T==0) = 1e-40;
            dX = -log10(abs(T))./numObservations;
            if max(isnan(dX(:)))
                dX = ones(size(dX),'like',dX);
            end
        end
    end
